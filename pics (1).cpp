 ...
 *
 * <#Read in a file, draw all the written shapes, and close file#>
 */
...
// Get a file name from user, add .bmp, and write the image to the file
void writeFile(const Graphics& drawer)
{
   
    string fileInput;
    cin >> fileInput;
    fileInput = fileInput + ".bmp";
    drawer.writeFile(fileInput);


    cout << "[Wrote " << fileInput << "]" << endl;

}

// open input file, Read the lines, 
void loadFile(Graphics& drawer)
{
    ifstream ins;
    ...
    while (ins >> letter) {
        if (letter == 'L') {
            ...
            Circle circleInfo;
            circleInfo.read(ins);
            circleInfo.draw(drawer);
            
        }
        else if (letter == 'T') {
            Triangle triangleInfo;
            triangleInfo.read(ins);
            triangleInfo.draw(drawer);
           
        }
        else if (letter == 'R') {
            Rectangle rectangleInfo;
            rectangleInfo.read(ins);
            rectangleInfo.draw(drawer);
           
        }
        else {
            drawer.clear();
            string unknownLine;
            getline(ins, unknownLine);
            cout << "Error in input file: " << letter << unknownLine << endl;
        }
    }
